...
var resources = {
	...
	Me: require('./resources/Me'),
    Vehicles: require('./resources/Vehicles'),
...
Uber.prototype.getAuthorizeUrl = function getAuthorizeUrl(scope, state) {
...
Uber.prototype.authorization = function authorization(options, callback) {
    ...
    this.oauth2.getOAuthAccessToken(code, {
        ...
        if (err) {
            ...
            return callback(null, [self.access_token, self.refresh_token, self.authorizedScopes, self.tokenExpiration, self.state]);
        }
    });
...
Uber.prototype.revokeToken = function revokeToken(options, callback) {
    var self = this;
    var code = '';
    var state = "";

    if (options.hasOwnProperty('access_token')) {
        code = options.access_token;
    } else {
        return callback(new Error('No access_token'));
    }
    ...
    var url = "https://login.uber.com/oauth/revoke";
    ...
    if (options && options.server_token) {
        ...
        if (!this.access_token) {
            ...
            if (this.isTokenStale()) {
                this.authorization({refresh_token : this.refresh_token }, function (err, res) {
                    if (err) {
                        return callback(err);
                    } else {
...
Uber.prototype.get = function get(options, callback, access_token) {
    var access_type = this.createAccessHeader(options.server_token);
    if(access_token) {
        access_type = "Bearer " + access_token;
    }
    if (!access_type) {
